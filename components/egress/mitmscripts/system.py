...
class ActiveVaultRevisionChanged(ActiveVaultLookupError):
    """The vault changed while resolving a dynamic credential binding."""
...
def _parse_active_vault(payload: Any) -> ActiveVault:
    ...
    for index, binding in enumerate(bindings):
        ...
        if binding.get("dynamic") is True:
            normalized_binding["dynamic"] = True
...
def _resolve_dynamic_binding(
    ...
    try:
        ...
        if response.status == 409:
            raise ActiveVaultRevisionChanged("active vault revision changed")
        if response.status != 200:
            raise ActiveVaultLookupError(
                f"dynamic credential lookup returned HTTP {response.status}"
            )
        resolved = _parse_active_vault(json.loads(body.decode("utf-8")))
        if resolved.revision != vault.revision:
            raise ActiveVaultRevisionChanged("active vault revision changed")
        if len(resolved.bindings) != 1 or resolved.bindings[0].get("name") != binding_name:
            raise ActiveVaultLookupError("dynamic credential lookup returned an invalid binding")
        return resolved.bindings[0], resolved.redactions
    except ActiveVaultLookupError:
        raise
    except Exception as exc:
        raise ActiveVaultLookupError(f"dynamic credential lookup failed: {exc}") from exc
    finally:
        with suppress(Exception):
            connection.close()
...
def requestheaders(flow: http.HTTPFlow) -> None:
    ...
    if binding.get("dynamic"):
        try:
            ...
            # Reload once when a mutation races with metadata selection.
            try:
                vault = _fetch_active_vault(_flow_client_ip(flow), None)
                if vault is None:
                    return
                binding = _select_binding(flow, vault)
                if not binding:
                ...
                if binding.get("dynamic"):
                    ...
                    dynamic_redactions = vault.redactions
            except ActiveVaultLookupError as exc:
                ctx.log.warn(f"credential proxy: {exc}; request denied")
            ...
            _reject_request(flow, b"credential proxy unavailable\n", status_code=503)
            return
        vault = ActiveVault(vault.revision, [binding], dynamic_redactions, vault.etag)
