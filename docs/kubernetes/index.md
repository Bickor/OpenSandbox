...
The optional Pod template is operator-controlled and can supply identity metadata, a ServiceAccount, resources, scheduling settings, and additional containers while the controller preserves the commit runtime invariants. The optional distroless `image-committer-azure` build variant uses `azidentity` to exchange an injected identity for ACR push credentials; see the [pause/resume guide](/guides/pause-resume#using-acr-with-azure-workload-identity).
